...
func newShimService(ctx context.Context, id string, publisher Publisher, shutdown func()) (Shim, error) {
	var (
		ep  oom.Watcher
		err error
	)
...
func (s *service) SystemdStartShim(ctx context.Context, opts StartOpts) (string, net.Listener, error) {
...
func (s *service) Cleanup(ctx context.Context, id string) (*taskAPI.DeleteResponse, error) {
...
func (s *service) Create(ctx context.Context, r *taskAPI.CreateTaskRequest) (_ *taskAPI.CreateTaskResponse, err error) {
	...
	logrus.WithField("id", r.ID).Info("Create")

	container, err := runc.NewContainer(ctx, s.platform, r)
	if err != nil {
		logrus.WithField("id", r.ID).WithError(err).Error("NewContainer Error")
		return nil, err
	}

	logrus.WithField("id", r.ID).Info("Create Success")
	...
	logrus.WithField("id", r.ID).Info("Create End")
...
func (s *service) shutdownAsync() {
	log.G(context.TODO()).Info("processes are all done, shutdown async")
	go func() {
		...
		s.mu.Unlock()
		for _, id := range ids {
			log.G(ctx).WithField("containerID", id).Info("kill container")

			resp, err := s.Delete(ctx, &taskAPI.DeleteRequest{
				ID: id,
			})
			if err != nil {
				log.G(ctx).WithField("containerID", id).WithError(err).Error("delete container error")
			}
			...
			if err = common.WriteExited(ctx, bundlePath, &exit); err != nil {
				log.G(ctx).WithField("containerID", id).WithError(err).Error("write exit status error")
			}
		}
...
func (s *service) checkProcesses(e goRunc.Exit) {
	log.G(context.TODO()).Info("check processes")
	defer log.G(context.TODO()).Info("done check processes")
	s.mu.Lock()
	defer s.mu.Unlock()

	livingProcessesCount := 0
	defer func() {
		if s.started && livingProcessesCount == 0 {
			s.shutdownAsync()
		}
	}()
...
func (s *service) forward(ctx context.Context, publisher Publisher) {
...
func (s *service) getContainer(id string) (*runc.Container, error) {
	logrus.WithField("id", id).Info("getContainer")
	s.mu.Lock()
	container := s.containers[id]
	s.mu.Unlock()

	if container == nil {
		logrus.WithField("id", id).Error("container not found")
