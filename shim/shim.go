...
// NewShimService returns a new shim service that can be used via GRPC
...
func (s *Service) Serve(ctx context.Context, sigTerm <-chan uint32) (err error) {
...
func (s *Service) serveTaskService(logger *logrus.Entry) (func(context.Context) error, error) {
	...
	shimapi.RegisterTaskService(server, &TaskService{service: s, logger: logger})
...
func (s *Service) checkProcesses(ctx context.Context, e goRunc.Exit) {
	...
	started := s.started()
	s.logger.Infof("check processes, started = %t, livingProcessesCount = %d", started, livingProcessesCount)
...
func (s *Service) cleanup(ctx context.Context) {
	...
	if err != nil {
		switch err {
		case ErrContainerDeleted:
			s.logger.Warn("container has already deleted")
		case ErrContainerNotCreated:
			s.logger.Warn("container not created")
		default:
...
func (s *Service) start(ctx context.Context) (pid uint32, err error) {
	...
	case *cgroupsv2.Manager:
		//nolint:nestif
		if allControllers, err := cg.RootControllers(); err == nil {
