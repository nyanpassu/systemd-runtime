...
func (t *Task) State(ctx context.Context) (state runtime.State, err error) {
	...
	defer func() {
		if err == nil {
			t.logger.Debugf("task state = %s", StatusIntoString(state.Status))
		}
	}()
...
func (t *Task) Kill(ctx context.Context, sig uint32, all bool) (err error) {
	...
	if sig != uint32(unix.SIGTERM) && sig != uint32(unix.SIGKILL) {
		ta, err := t.connMng.getRunningService(ctx)
		if err != nil {
			return err
		}
		return ta.Kill(ctx, sig, all)
	}
	...
	if err != nil {
		logger.WithError(err).Error("[Task Kill] disable bundle error")
		return err
	}

	if !running {
		t.connMng.Disabled(status.Exit)
		if err := t.exchange.Publish(ctx, runtime.TaskExitEventTopic, &events.TaskExit{
			ID:          t.ID(),
			ContainerID: t.ID(),
			Pid:         0,
			ExitStatus:  status.Exit.Status,
			...
			logger.WithError(err).Error("[Task Kill] publish task exit event failed")
		}
		return nil
	}
	t.connMng.Disable()
...
// CloseIO closes the processes stdio
...
func (t *Task) Delete(ctx context.Context) (exit *runtime.Exit, err error) {
	...
	if sub.exit != nil {
		...
		if err := t.exchange.Publish(ctx, runtime.TaskDeleteEventTopic, &events.TaskDelete{
			ID:          t.ID(),
			ContainerID: t.ID(),
			Pid:         0,
			...
			t.logger.WithError(err).Error("publish task delete event failed")
		}
		return sub.exit, nil
	}
