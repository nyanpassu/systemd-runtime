...
func (b *Bundle) Cleanup(ctx context.Context) error {
	...
	log.G(ctx).WithField("created", status.Created).WithField("disabled", status.Disabled).WithField("id", b.id).WithField("namespace", b.namespace).Debug("check status during cleanup")
	if status.Created && !status.Disabled {
		log.G(ctx).WithField("id", b.id).WithField("namespace", b.namespace).Debug("publish pause event")
		if err := b.exchange.Publish(namespaces.WithNamespace(ctx, b.namespace), runtime.TaskPausedEventTopic, &events.TaskPaused{
			ContainerID: b.ID(),
		}); err != nil {
			log.G(ctx).WithError(err).Error("publish paused event failed")
		}
	}
