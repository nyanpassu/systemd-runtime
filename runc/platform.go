...
func NewPlatform() (stdio.Platform, error) {
	...
	go epoller.Wait() //nolint:errcheck
...
func (p *linuxPlatform) CopyConsole(ctx context.Context, console console.Console, id, stdin, stdout, stderr string, wg *sync.WaitGroup) (cons console.Console, retErr error) {
	...
	if stdin != "" {
		...
		go func() {
			...
			io.CopyBuffer(epollConsole, in, *bp) //nolint:errcheck
			// we need to shutdown epollConsole when pipe broken
			epollConsole.Shutdown(p.epoller.CloseConsole) //nolint:errcheck
			epollConsole.Close()
		}()
	}
	...
	case "binary":
		...
		go func() {
			cwg.Done()
			io.Copy(outW, epollConsole) //nolint:errcheck
	...
	default:
		...
		go func() {
			...
			io.CopyBuffer(outw, epollConsole, *buf) //nolint:errcheck
