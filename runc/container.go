...
func newInit(_ context.Context, path, workDir, namespace string, platform stdio.Platform,
	r *process.CreateConfig, options *options.Options, rootfs string) *process.Init {
...
func (c *Container) Start(ctx context.Context, execID string) (process.Process, error) {
...
func (c *Container) Delete(ctx context.Context, execID string) (process.Process, error) {
	p, err := c.Process(execID)
	if err != nil {
		return nil, err
	}
	...
	if execID != "" {
		c.ProcessRemove(execID)
	}
	return p, nil
}

// Exec an additional process
func (c *Container) Exec(ctx context.Context, r *process.ExecConfig) (process.Process, error) {
	process, err := c.process.(*process.Init).Exec(ctx, c.Bundle, r)
	if err != nil {
		return nil, err
	}
...
func (c *Container) ResizePty(ctx context.Context, execID string, width uint16, height uint16) error {
	p, err := c.Process(execID)
	if err != nil {
		return err
	}
...
// Kill a process
func (c *Container) Kill(ctx context.Context, execID string, signal uint32, all bool) error {
	p, err := c.Process(execID)
	if err != nil {
		return err
	}
...
func (c *Container) CloseIO(ctx context.Context, execID string) error {
	p, err := c.Process(execID)
	if err != nil {
		return err
	}
...
func (c *Container) Checkpoint(ctx context.Context, config *process.CheckpointConfig) error {
	p, err := c.Process("")
	if err != nil {
		return err
	}
	return p.(*process.Init).Checkpoint(ctx, config)
}

// Update the resource information of a running container
func (c *Container) Update(ctx context.Context, resources *ptypes.Any) error {
	p, err := c.Process("")
	if err != nil {
		return err
	}
	return p.(*process.Init).Update(ctx, resources)
}
